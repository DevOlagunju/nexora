...
export async function submitKycAction(formData: FormData): Promise<ActionResult> {
  ...
  await writeAudit("kyc.submit", {
    userId: user.id,
    meta: { bvnLast4: bvn.slice(-4), ninLast4: nin.slice(-4) },
  });

  return { ok: true, message: "KYC submitted for review. We never store full BVN/NIN." };
}
...
export async function updateCryptoOrderAction(formData: FormData): Promise<ActionResult> {
  ...
  if (parsed.data.status === "COMPLETED" || parsed.data.status === "PAYOUT_SENT") {
  ...
  await writeAudit("admin.crypto_status", {
    userId: admin.id,
    meta: { id: parsed.data.id, status: parsed.data.status },
  });
  return { ok: true, message: "Crypto order updated." };
}
...
export async function updateGiftOrderAction(formData: FormData): Promise<ActionResult> {
  ...
  return { ok: true, message: "Gift card order updated." };
}
